...
{
    CCArray* ccproducts = CCArray::arrayWithCapacity(response.products.count);
    ...
    CCArray* ccinvalidProductsId = NULL;
    if (response.invalidProductIdentifiers.count > 0)
    {
        ccinvalidProductsId = CCArray::arrayWithCapacity(response.invalidProductIdentifiers.count);
...
- (void)verifyTransactionReceipt:(SKPaymentTransaction *)transaction
{
    ...
    int dataUsed = CCCrypto::encodeBase64([transaction.transactionReceipt bytes], length, buffer, 2 * length);
    if (dataUsed <= 0)
    {
    ...
    if (isSandbox_)
    {
        urlString = [NSString stringWithUTF8String:SANDBOX_RECEIPT_VERIFY_URL];
    }
    else
    {
        urlString = [NSString stringWithUTF8String:APPSTORE_RECEIPT_VERIFY_URL];
    }
    NSURL *url = [NSURL URLWithString:urlString];
    
    CCStoreReceiptVerifyRequest_objc *request = [[CCStoreReceiptVerifyRequest_objc alloc] initWithURL:url];
    request.productIdentifier = transaction.payment.productIdentifier;
...
{
    CCLOG("[CCStore_obj] verify receipt requestFinished() pid: %s\n  response:\n%s\n\n",
          utf8cstr(request.productIdentifier),
          utf8cstr(request.responseString));
    ...
    {
        // invalid JSON string
        [self transactionFailed:request.transaction
         andReceiptVerifyStatus:CCStoreReceiptVerifyStatusInvalidResult];
    }
    else if ([[jsonData objectForKey:@"status"] isKindOfClass:[NSString class]]
             && [[jsonData objectForKey:@"status"] intValue] == 0)
    {
        ...
        do
        {
            ...
            SKPaymentTransaction *transaction = request.transaction;
        ...
        if (isValidReceipt)
        {
            [self transactionCompleted:request.transaction
...
{
    CCLOG("[CCStore_obj] WARN, verify receipt requestFailed() pid: %s\n  response:\n%s\n\n",
          utf8cstr(request.productIdentifier),
          utf8cstr(request.responseString));
...
- (CCStorePaymentTransaction *)createCCStorePaymentTransaction:(SKPaymentTransaction *)transaction
                                        andReceiptVerifyStatus:(int)receiptVerifyStatus
{
    CCStorePaymentTransactionWrapper* transactionWrapper;
    transactionWrapper = CCStorePaymentTransactionWrapper::createWithTransaction(transaction);
    ...
    if (transaction.originalTransaction)
    {
        ...
        return CCStorePaymentTransaction::transactionWithState(transactionWrapper,
                                                               ccstate,
                                                               ccid,
                                                               ccproductId,
                                                               quantity,
